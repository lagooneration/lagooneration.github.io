...
void main(){
	vec3 color = vec3(0.0);
    vec3 glowColor = vec3(0.0, 0.4, 1.0);
    float glowIntensity = 80.0;
    float diff = dot(vec3(0.0,0.4,1.0), vNormal);
    ...
    // vec3 glow = glowIntensity * glowColor / (distance(gl_FragCoord.xy, vec2(0.5, 0.5)) * 0.02);
    

    // gl_FragColor = vec4(glow, 1.0);
    // gl_FragColor = vec4(vNormal, 1.0);
    gl_FragColor = vec4(sin(diff*10.0));
    // vec3 color = mix(vec3(sin(u_time)),glow,0.5);

    // color = color + vec3(sin(diff*10.0));
